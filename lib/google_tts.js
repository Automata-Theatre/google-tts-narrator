...
class GoogleTTS {
  ...
  async * yieldSpeeches() {
    ...
    while (true) {
      ...
      value['input']['text'] = textList[this.step - 1]
      response['narratorInfo'] = {
        step: this.step, totalCount: this.totalCount, info: value,
      }
  ...
  async fetchSpeeches() {
    ...
    const results = []
    const requestData = this.requestDataList
    for (let i = 0; i < requestData.length; i++) {
      const value = requestData[i];
      const [response] = await this.client.synthesizeSpeech(value)
      this.step = i + 1
